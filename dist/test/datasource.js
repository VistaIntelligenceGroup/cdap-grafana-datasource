...
exports.CDAPDatasource = undefined;
...
var CDAPDatasource = exports.CDAPDatasource = function () {
  ...
  _createClass(CDAPDatasource, [{
    ...
    value: function metricFindQuery(query) {
      ...
      return this.searchForMetrics().then(function (metrics) {
        return _lodash2.default.map(metrics, function (x) {
    ...
    value: function searchForTags(tags) {
      ...
      return this._metricsSearch('tag', tags).then(function (objs) {
        var names = _lodash2.default.sortBy(_lodash2.default.uniq(_lodash2.default.map(objs, function (x) {
          return x.name;
        })));
        var found = _lodash2.default.filter(names, function (x) {
          return x !== 'namespace';
        });
        var stars = _lodash2.default.map(found, function (x) {
          return { name: x, value: '*' };
        });
        return _lodash2.default.concat(stars, objs);
      }).then(function (objs) {
        return _lodash2.default.map(objs, function (x) {
          return x.name + ':' + x.value;
        });
      }).then(function (strs) {
        return _lodash2.default.sortBy(strs);
      });
    }
  }, {
    ...
    value: function query(options) {
      ...
      var data = _lodash2.default.fromPairs(_lodash2.default.map(options.targets, function (target) {
        var metrics = target.metrics || [];
        return [target.refId || defaultRefId, {
          tags: _lodash2.default.fromPairs(_lodash2.default.map(target.tags, function (str) {
            return str.split(':');
          })),
      ...
      _lodash2.default.forEach(options.targets, function (target) {
        _lodash2.default.forEach(_lodash2.default.uniq(target.metrics), function (metric) {
      ...
      return this._cdapRequest('/v3/metrics/query', {}, data).then(function (response) {
        return { data: _lodash2.default.flatMap(options.targets, function (target) {
            ...
            return _lodash2.default.map(response.data[refId].series, function (found) {
              ...
              _lodash2.default.forEach(target.groups, function (group) {
              ...
              return {
                ...
                datapoints: _lodash2.default.map(found.data, function (pt) {
